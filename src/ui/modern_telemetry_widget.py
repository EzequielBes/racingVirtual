...
class ModernTelemetryWidget(QWidget):
    ...
    def load_telemetry_data(self, data: Dict[str, Any]):
        ...
        try:
            ...
            elif 'data' in data and hasattr(data['data'], 'shape'):  # Verifica se é DataFrame
    ...
    def _update_metrics_from_beacons(self, beacons: List[Dict[str, Any]]):
        """Atualiza métricas a partir dos beacons."""
        if not beacons:
            return
            
        try:
            # Pega o último beacon para dados atuais
            latest = beacons[-1]
            
            # Atualiza valores
            if 'lap_time' in latest:
                lap_time = latest['lap_time']
                self.current_lap_card.update_value(f"{lap_time:.3f}")
            
            # Calcula melhor tempo
            times = [b.get('lap_time', 0) for b in beacons if b.get('lap_time', 0) > 0]
            if times:
                best_time = min(times)
                self.best_lap_card.update_value(f"{best_time:.3f}")
            
            logger.info("Métricas atualizadas a partir dos beacons")
            
        except Exception as e:
            logger.error(f"Erro ao atualizar métricas dos beacons: {e}")

    def _update_metrics_from_dataframe(self, df):
        """Atualiza métricas a partir do DataFrame."""
        ...
        try:
            ...
            logger.info("Métricas atualizadas a partir do DataFrame")
            
        except Exception as e:
            logger.error(f"Erro ao atualizar métricas do DataFrame: {e}")
